...
impl SccContent {
    pub fn is_node_in(&self, node: NodeIndex<usize>) -> bool {
        match self {
            SccContent::SubScc(it) => it.is_node_in(node),
            SccContent::Node(it) => node.index() == *it,
        }
    }
}
...
impl Scc {
    pub fn new(
        ...
        if sccs.len() == 1 {
            return Self {
                ...
                content: sccs[0]
                    .iter()
                    .map(|it| SccContent::Node(it.index()))
                    .collect(),
            };
        }
        let mut content: Vec<_> = sccs
            ...
            .map(|mut scc| {
                if scc.len() == 1 {
                    SccContent::Node(scc.pop().unwrap().index())
                } else {
                    let sub_loop = Scc::new(graph, &scc, &new_backedges);
                    SccContent::SubScc(Box::new(sub_loop))
                }
            })
            .collect();
        for entry in &entries {
            if !content.iter().any(|it| it.is_node_in(*entry)) {
                content.push(SccContent::Node(entry.index()));
            }
        }
        Self {
            entries: entries.into_iter().map(|it| it.index()).collect(),
        ...
        if self.entries.len() > 1 {
            ...
            self.content
                ...
                .filter_map(|it| match it {
                    SccContent::SubScc(sub_loop) => Some(sub_loop),
    ...
    pub fn smallest_loop_node_in(&self, node: NodeIndex<usize>) -> Option<&Scc> {
        let found_node = self
            .content
            .iter()
            .filter_map(|it| {
    ...
    pub fn is_node_in(&self, node: NodeIndex<usize>) -> bool {
        self.content.iter().any(|it| match it {
            SccContent::SubScc(sub_loop) => sub_loop.is_node_in(node),
            SccContent::Node(n) => node.index() == *n,
        })
    }
    ...
    pub fn node_count(&self) -> usize {
        self.content
            .iter()
            .map(|it| match it {
                SccContent::SubScc(sub_loop) => sub_loop.node_count(),
                SccContent::Node(_) => 1,
            })
            .sum()
    }
