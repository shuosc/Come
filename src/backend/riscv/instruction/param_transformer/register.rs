use crate::backend::riscv::instruction::param::Param;
use bitvec::prelude::*;
...
impl IsParamTransformer for Register {
    ...
    fn update_param(&self, instruction_part: &BitSlice<u32>, param: &mut Param) {
        if let Param::Register(param_value) = param {
...
mod tests {
    ...
    fn test_argument_to_bits() {
        ...
        let bits = transformer.param_to_instruction_part(0, &Param::Register(0x1f));
    ...
    fn test_update_argument() {
        let transformer = Register::new();
        let mut param = Param::Register(0);
        transformer.update_param(bits![u32, Lsb0; 1, 1, 1, 1, 1], &mut param);
        assert_eq!(param, Param::Register(0x1f));
    }
}
